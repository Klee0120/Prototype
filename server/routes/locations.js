...
// general (E&F) time across every location -- unlike the E&F Contract Job
// Number and WOM subsidiary codes, which vary per location/project and are
// stored on the location/WOM records themselves.
...
router.post("/", requireAuth, requireAdmin, (req, res) => {
  const { code, name, efJobNumber, region } = req.body || {};
  if (!code || !name) return res.status(400).json({ error: "code and name are required" });
  if (db.findLocation(code)) return res.status(409).json({ error: "Location code already exists" });

  db.createLocation(code, name, efJobNumber || null, region || null);
...
router.patch("/:code", requireAuth, requireAdmin, (req, res) => {
  const { name, efJobNumber, region } = req.body || {};
  const existing = db.findLocation(req.params.code);
  if (!existing) return res.status(404).json({ error: "Location not found" });
  if (!name) return res.status(400).json({ error: "name is required" });

  const location = db.setLocationDetails(req.params.code, { name, efJobNumber, region });
