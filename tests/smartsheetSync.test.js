...
test("smartsheet sync: creates, promotes, and updates WOMs by underlying row", async (t) => {
  ...
  await t.test("RFM approving a pending WOM to send to Toyota makes it allocatable, even with no real WOM # yet", async () => {
    const restore = stubFetchOnce({
      ok: true,
      json: async () =>
        sheetWith([
    ...
    try {
      const sync = await server.call("POST", "/api/admin/smartsheet/sync-woms", { userId: "ADMIN" });
      assert.equal(sync.status, 200);

      const meta = await server.call("GET", "/api/meta/current-week");
      const week = meta.body.weekMonday;

      // Still just "pending" -- RFM hasn't approved sending it to Toyota, so
      // a technician can't charge time to it yet.
      const blocked = await server.call("PUT", `/api/technicians/T1001/weeks/${week}/allocations`, {
        userId: "T1001",
        body: { allocations: [{ day: "Mon", type: "wom", locationCode: "PRINCETON", womCode: "PENDING-601", hours: 8 }] },
      });
      assert.equal(blocked.status, 400);

      // RFM decides to send it to Toyota -- flips it to "requested" by hand
      // (same generic status-change endpoint as any other status) and gives
      // it a location so a technician can allocate against it.
      await server.call("PATCH", "/api/woms/PENDING-601/details", {
        userId: "ADMIN",
        body: { description: "Gutter replacement", locationCode: "PRINCETON" },
      });
      const approve = await server.call("PATCH", "/api/woms/PENDING-601", { userId: "ADMIN", body: { status: "requested" } });
      assert.equal(approve.status, 200);
      assert.equal(approve.body.status, "requested");

      // Now a technician CAN charge time to it, despite there being no real
      // WOM # yet.
  ...
  await t.test("a 'requested' WOM getting a real WOM # from a later sync promotes it too", async () => {
    const restore = stubFetchOnce({
      ok: true,
      json: async () =>
        sheetWith([
